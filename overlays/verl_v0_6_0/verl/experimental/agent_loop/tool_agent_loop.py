...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        if (
            ...
            and agent_data.interaction_kwargs.get("name") == "propose_refine"
            and agent_data.interaction_kwargs.get("train_proposal", False) is not True
            and agent_data.response_mask
            and not any(agent_data.response_mask)
        ):
            # If proposal parsing fails, no refine turn is produced. A fully
            # masked single-sample PPO microbatch can create a zero-token loss
            # denominator, so keep the failed proposal trainable with reward=0.
            agent_data.response_mask = [1] * len(agent_data.response_mask)
            agent_data.metrics["propose_refine_unmasked_failed_proposal"] = 1.0
        ...
        output.extra_fields["assistant_responses"] = agent_data.assistant_responses
        return output
    ...
    async def _handle_generating_state(
        ...
        train_generated_tokens = 1
        if (
            self.interaction_config_file
            and agent_data.interaction_kwargs.get("name") == "propose_refine"
            and agent_data.interaction_kwargs.get("train_proposal", False) is not True
            and agent_data.assistant_turns == 1
        ):
            # The first assistant turn is proposal. It builds the refine prompt,
            # but the RL objective here is refine-only, so mask it out of PPO loss.
            train_generated_tokens = 0
        agent_data.response_mask += [train_generated_tokens] * len(agent_data.response_ids)
        if output.log_probs:
        ...
        if self.interaction_config_file:
            ...
            agent_data.assistant_responses.append(assistant_message)
